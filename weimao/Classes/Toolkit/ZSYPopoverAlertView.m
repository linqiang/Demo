...
#import "ZSYPopoverAlertView.h"
#import <QuartzCore/QuartzCore.h>
#import <objc/runtime.h>

static const CGFloat ZSYCustomButtonHeight = 40;

static const char * const kZSYPopoverAlertButtonClickForCancel = "kZSYPopoverListButtonClickForCancel";

static const char * const kZSYPopoverAlertButtonClickForDone = "kZSYPopoverListButtonClickForDone";

@interface ZSYPopoverAlertView ()
@property (nonatomic, retain) UIView *mainPopoverListView;                 //主的选择列表视图
...
@implementation ZSYPopoverAlertView
...
- (void)initTheInterface
{
    ...
    CGRect tableFrame = CGRectMake(0, 45.0f, xWidth, self.bounds.size.height-45.0f);
    _mainPopoverListView = [[UIView alloc] initWithFrame:tableFrame ];
...
- (void)setCancelButtonTitle:(NSString *)aTitle block:(ZSYPopoverAlertViewButtonBlock)block
{
    if (nil == _cancelButton)
    {
        self.cancelButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.cancelButton setBackgroundImage:[ZSYPopoverAlertView cancelButtonBackgroundImage] forState:UIControlStateNormal];
    ...
    objc_setAssociatedObject(self.cancelButton, kZSYPopoverAlertButtonClickForCancel, [block copy], OBJC_ASSOCIATION_RETAIN);
}

- (void)setDoneButtonWithTitle:(NSString *)aTitle block:(ZSYPopoverAlertViewButtonBlock)block
{
    if (nil == _doneButton)
    {
        self.doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.doneButton setBackgroundImage:[ZSYPopoverAlertView normalButtonBackgroundImage] forState:UIControlStateNormal];
    ...
    objc_setAssociatedObject(self.doneButton, kZSYPopoverAlertButtonClickForDone, [block copy], OBJC_ASSOCIATION_RETAIN);
}
#pragma mark - UIButton Clicke Method
- (void)buttonWasPressed:(id)sender
{
    UIButton *button = (UIButton *)sender;
    ZSYPopoverAlertViewButtonBlock __block block;
    
    if (button == self.cancelButton)
    {
        block = objc_getAssociatedObject(sender, kZSYPopoverAlertButtonClickForCancel);
    }
    else
    {
        block = objc_getAssociatedObject(sender, kZSYPopoverAlertButtonClickForDone);
    }
    if (block)
    {
        block();
